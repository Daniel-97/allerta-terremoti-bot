...
describe("selectZone", () => {
  it("selects lazio for Roma (41.9, 12.5)", () => {
    const z = selectZone(41.9, 12.5);
    expect(z.id).toBe("lazio");
  });

  it("selects lombardia for Milano (45.46, 9.19)", () => {
    const z = selectZone(45.46, 9.19);
    expect(z.id).toBe("lombardia");
  });
  ...
  it("selects campania for Napoli (40.85, 14.27)", () => {
    const z = selectZone(40.85, 14.27);
    expect(z.id).toBe("campania");
  });

  it("selects calabria (not sicilia) for Reggio Calabria, despite both boxes containing it", () => {
    const z = selectZone(38.1113, 15.6494);
    expect(z.id).toBe("calabria");
  });
...
describe("latLonToPixel", () => {
  const lazio = zones.find((z) => z.id === "lazio")!;

  it("returns values within image bounds for Roma", () => {
    const { x, y } = latLonToPixel(41.9, 12.5, lazio);
    expect(x).toBeGreaterThanOrEqual(0);
    expect(x).toBeLessThanOrEqual(lazio.width);
    expect(y).toBeGreaterThanOrEqual(0);
    expect(y).toBeLessThanOrEqual(lazio.height);
  });

  it("returns 0,0 for top-left corner of zone", () => {
    const { x, y } = latLonToPixel(lazio.maxLatitude, lazio.minLongitude, lazio);
    expect(x).toBe(0);
    expect(y).toBe(0);
  });

  it("returns width,height for bottom-right corner of zone", () => {
    const { x, y } = latLonToPixel(lazio.minLatitude, lazio.maxLongitude, lazio);
    expect(x).toBe(lazio.width);
    expect(y).toBe(lazio.height);
  });
});
...
describe("buildOverlaySvg", () => {
  ...
  it("includes viewBox dimensions matching zone", () => {
    const svg = buildOverlaySvg(lazio, 300, 300);
    expect(svg).toContain(`width="${lazio.width}"`);
    expect(svg).toContain(`height="${lazio.height}"`);
  });
